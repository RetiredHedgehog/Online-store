...
export default function createDoubleSlider({className, text, cart, sortingField} : {className: string, text: string, cart: Cart, sortingField: 'price' | 'stock'}) {
  const wrapper = document.createElement('div');
  wrapper.id = className;
  wrapper.classList.add(className);

  cart.productsFetched.sort((a: productItem, b: productItem) => a[sortingField] - b[sortingField]);
