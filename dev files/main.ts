...
function RangeWidth(arrPrice: any) {
    ...
    console.log((+price[1] - +price[0]) / 17.49 + '%')
}
...
function search(text: string, arr: any) {
    arr.map((item:any) => {
        if(item.brand.toLowerCase().indexOf(text) > -1 || item.category.toLowerCase().indexOf(text) > -1 || item.title.toLowerCase().indexOf(text) > -1){
            inpArrSearch.push(item)
        }
    })
    // newItemsArray = inpArrSearch
}
...
newItemsArray = [];
    mainContainerItemArray.forEach((item: any) => {
        ...
        inpArrSearch.forEach((item1: any) => {
           ...
           if (item1.title.toLowerCase() == arr[4].toLowerCase()){
            item.style.display = '';
            newItemsArray.push(item)
           }
        })
...
function inputNumberCurrentValueGenerator(arr: any) {
    ...
    inputMin.value =price[0] || priceArr[0]
    inputMax.value =price[1] || priceArr[priceArr.length - 1]
    inputRangeMin.value =price[0] || priceArr[0]
    inputRangeMax.value =price[1] || priceArr[priceArr.length - 1]
    progress.style.left = (+price[0] / 1749) * 100 + '%'|| (priceArr[0] / 1749) * 100 + '%';
    progress.style.right = (1 - (+price[1] / 1749)) * 100 + '%'|| (1 - (priceArr[priceArr.length - 1] / 1749)) * 100 + '%';
    progress.style.width =  (+price[1] - +price[0]) / 17.49 + '%'||(priceArr[priceArr.length - 1] - priceArr[0])/ 17.49 + '%';
...
function inputNumberCurrentStockValueGenerator(arr: any) {
    ...
    priceArr.sort((a: number ,b: number ) => a > b ? 1 : -1)
    inputMinStock.value = stock[0] || priceArr[0]
    inputMaxStock.value =stock[1] || priceArr[priceArr.length - 1]
    inputsRangeStockmin.value =stock[0] || priceArr[0]
    inputsRangeStockmax.value =stock[1] || priceArr[priceArr.length - 1]
    progressStock.style.left =(+stock[0] / 150) * 100 + '%' || (priceArr[0] / 150) * 100 + '%';
    progressStock.style.right =( 1 - (+stock[1] / 150)) * 100 + '%' || (1 - (priceArr[priceArr.length - 1] / 150)) * 100 + '%';
    progressStock.style.width = (+stock[1] - +stock[0]) / 1.5 + '%' || (priceArr[priceArr.length - 1] - priceArr[0])/ 1.5 + '%';
}
